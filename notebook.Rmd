...
Macierz korelacji doskonale sprawdza się do identyfikowania zależności między różnymi atrybutami (cechami) danych. Wykorzystamy ją aby sprawdzić współczynnik korelacji między naszymi atrybutami. 
Wartość tego współczynnika mieści się w przedziale od -1 do 1, gdzie:
...
Współczynnik korelacji 0.9283 wskazuje na bardzo silny związek dodatni 
między gęstością energii grawimetrycznej a wolumetrycznej. 
Oznacza to, że materiały, które mogą zmagazynować więcej energii 
w stosunku do swojej masy, zazwyczaj mogą również zmagazynować więcej 
energii w stosunku do swojej objętości.
...
Na podstawie współczynnika korelacji między pojemnością grawimetryczną a 
pojemnością wolumetryczną (0.8584), możemy stwierdzić, że istnieje silna, 
dodatnia zależność między tymi dwoma parametrami.
```{r n7_GC_VC}
...
Współczynnik korelacji -0.2128 między średnim napięciem a pojemnością wolumetryczną 
wskazuje na słabą, ujemną korelację między tymi dwoma parametrami. 

```{r n7_AV_VC}
correlation_coefficient_av_vc <- cor(mydata$Average.Voltage, mydata$Volumetric.Capacity, use = "complete.obs")

print(paste("Współczynnik korelacji -> Average Voltage and Volumetric Capacity:", correlation_coefficient_av_vc))

ggplot(mydata, aes(x = Average.Voltage, y = Volumetric.Capacity)) +
  geom_point(fill = "lightblue", color = "black") +
  geom_smooth(method = "lm", col = "darkred") +
  theme_minimal() +
  labs(title = "Average Voltage vs Volumetric Capacity",
       x = "Average Voltage (V)",
       y = "Volumetric Capacity (mAh/cm³)") +
  xlim(-10, 20) +
  ylim(-10, 7300)
...
Współczynnik korelacji 0.6180 między pojemnością wolumetryczną a 
ułamkiem atomowym rozładowania wskazuje na umiarkowaną, 
dodatnią zależność między tymi dwoma parametrami. 
Oznacza to, że ogólnie, materiały o większej pojemności wolumetrycznej 
mają tendencję do większego ułamka atomów zaangażowanych w proces rozładowania.
...
W przypadku predykcji stabilności rozładowania, wykorzystamy las losowy (random forest).
...
trainIndex_discharge <- createDataPartition(response_discharge, p = .8, 
                                            list = FALSE, 
                                            times = 1)
trainData_discharge <- mydata[trainIndex_discharge,]
testData_discharge <- mydata[-trainIndex_discharge,]
tunegrid <- expand.grid(.mtry = c(2, 3, 4))

model_discharge <- train(Stability.Discharge ~ Average.Voltage + Volumetric.Capacity, 
                         data = trainData_discharge, 
                         method = "rf",
                         tuneGrid = tunegrid,
                         ntree=10)

#print(summary(model_discharge))

predictions_discharge <- predict(model_discharge, newdata = testData_discharge)

#postResample(predictions_discharge, testData_discharge$Stability.Discharge)
evaluation <- postResample(predictions_discharge, testData_discharge$Stability.Discharge)
...
ggplot(testData_discharge, aes(x = Stability.Discharge, y = predictions_discharge)) +
  geom_point(color = "black") +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  theme_minimal() +
...
ggplot(mapping = aes(alpha = 0.4)) + 
  geom_density(aes(x = Stability.Discharge, fill = "Actual"), data = testData_discharge) + 
  geom_density(aes(x = predictions_discharge, fill = "Predicted"), data = testData_discharge) + 
  theme_minimal() +
